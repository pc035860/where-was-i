...
- **Workspace slug resolution** — both Claude (encodedPath fallback) and Cursor (no `.workspace-trusted`) use `resolveWorkspaceSlug()` in `project-name.ts` to reconstruct real paths from dash-encoded slugs. Claude's encodedPath has a leading `-` which the scanner strips before resolving. `resolveSegments()` walks shortest-first, checking dash-joined then space-joined candidates against the filesystem. **Known limitation**: no backtracking — prefix collisions (e.g., both `~/code/` and `~/code-gen/` exist) may resolve incorrectly
- **Cursor project info** comes from `.workspace-trusted` JSON file (`workspacePath` field) in the workspace directory; fallback goes through slug resolution above
